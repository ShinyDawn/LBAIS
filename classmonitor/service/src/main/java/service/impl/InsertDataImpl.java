...
public class InsertDataImpl implements InsertDataService {
    ...
    // 插值周数
    private static final int week = 12;
...
//                    String date = DateUtil.getFutureDate(i * 7 + j + 1, DateUtil.getDate());
//                    int day = 0;
//                    try {
//                        day = DateUtil.dayForWeek(date);
//                        if (day == 6 || day == 7) {
//                            continue;
//                        }
//                    } catch (Exception e) {
...
//                    c.setDay(day);
//                    c.setTid(k + 1);
//                    c.setCourse(curriculum[day - 1][k]);
...
//				String date = format.format(cal.getTime());
                String date = DateUtil.getPassedDate(i * 7 + j - 1, DateUtil.getDate());
                int day = 0;
                try {
                    day = DateUtil.dayForWeek(date);
                    if (day == 6 || day == 7) {
                ...
                for (int k = 0; k < curriculum[j - 1].length; k++) {
                    for (int n = 0; n < student.length; n++) {
                        for (int m = 0; m < max_frequency; m++) {
                            ...
                            if (random < 0.7 && !curriculum[j - 1][k].equals("自习")) {
